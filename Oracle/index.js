...
slots = new Array(10);
console.log(slots);
demand = new Array(letterAddresses.length);
for (c = 0; c < alphabet.length; c++) {
  demand[c] = new Array();                                                                           /* Create matrix to store demand data points */
  demand[c].push(0);
  LetterMonitor.at(spawner.deployedLettersMonitors()[c]).setProductionTime(getRandomInt(1, 10));     /* Set the production time of the letters to some random number between 1 and 10 */
}
...
lineReader.on('line', function (line) {
  ...
  for (w = 0; w < words.length; w++) {                            /* Loop through all words in current line */
    ...
    history = child_process.execSync('python input.py -t ' + JSON.stringify(demand) + JSON.stringify(slots));
